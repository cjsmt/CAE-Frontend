  ...
  <div>
    ...
    <el-table :data="userList" style="width: 100%" stripe border>
      ...
      <el-table-column label="操作" align="center">
        ...
        <template v-slot="{ row }">
          <!-- <a href="#">详情</a>&nbsp; -->
          <el-button><router-link :to="'/users/' + row.id">详情</router-link>&nbsp;</el-button>
          <el-button><a href="#" @click.prevent="onRemove(row.id)">删除</a></el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    >
      ...
      <el-form :model="form" :rules="formRules">
        <el-form-item label="位置" label-width="80px" prop="name">
          <el-input v-model="form.position" placeholder="请输入您要用力的位置"></el-input>
        </el-form-item>

        <el-form-item label="压力大小" label-width="80px" prop="age">
          <el-input
            v-model.number="form.age"
            placeholder="请输入用力的大小"
          ></el-input>
        </el-form-item>

        <el-form-item label="用户昵称" label-width="80px" prop="position">
  ...
  data() {
    ...
    let checkAge = (rule, value, cb) => {
      if (!Number.isInteger(value)) {
        return cb(new Error("请填写坐标"));
      }
      if (value > 100 || value < 1) {
        return cb(new Error("大小必须在1-100之间"));
      }
      // 验证通过：直接调用cb();
      cb();
    };
    return {
      ...
      formRules: {
        name: [
          { required: true, message: "位置是必填项", trigger: "blur" },
          { min: 1, max: 15, message: "长度在3-10之间", trigger: "blur" },
        ],
        age: [
          { required: true, message: "压力是必填项", trigger: "blur" },
